...
QR_IMAGE_URL = "https://i.ibb.co/bRCJswh4/file-70.jpg"

# ============== UPI INFO ==============
UPI_ID = "BHARATPE09905322322@yesbankltd"
...
def get_demo_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🎥 CHECK YOUR CATAGORY", url="https://t.me/+l2gXxC8YII04NzZl")],
...
async def handle_get_premium(query, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    caption = (
        ...
        "➖➖➖➖➖➖➖➖➖➖\n"
        "*STEPS:*\n"
        "1️⃣ Scan QR or pay via UPI ID\n"
        "2️⃣ Pay exactly ₹59\n"
        "3️⃣ Take screenshot of success\n"
        "4️⃣ Send screenshot HERE in chat\n\n"
        "⏳ *Admin will verify & send group link!*"
    )
    ...
    try:
        ...
        # Fallback: send as text so user can still pay
        try:
            await context.bot.send_message(
                chat_id=chat_id,
                text=caption,
            ...
            logger.info(f"[PREMIUM] Sent fallback text to user {user_id}")
        except Exception as e2:
            logger.error(f"[PREMIUM] Fallback also failed: {e2}")
            awaiting_screenshot.discard(user_id)
            await query.message.reply_text(
                "❌ Something went wrong. Please contact @VIDEOSELLER50R",
                reply_markup=get_main_keyboard()
            )
